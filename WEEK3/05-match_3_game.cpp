...
int main() {
	...
	while(true) {
		...
		if(inputX-inputSwapX<=1 && inputX-inputSwapX>=-1 && inputY-inputSwapY<=1 && inputY-inputSwapY>=-1){
		grid[inputX][inputY] = grid[inputSwapX][inputSwapY];
		grid[inputSwapX][inputSwapY] = temp;} else {
				cout << "You can only swap with ADJACENT positions!" << "\n\n";
			}
		...
		for (int i = -1; i < 2; i++) {
			...
			char temp2 = grid[inputX][inputY];
			grid[inputX][inputY] = grid[inputSwapX][inputSwapY];
			grid[inputSwapX][inputSwapY] = temp2;
		}
